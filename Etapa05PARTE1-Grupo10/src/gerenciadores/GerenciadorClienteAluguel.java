...
public class GerenciadorClienteAluguel implements GerenciadorClientes {
	...
	//Construtor
	private GerenciadorClienteAluguel() {
	...
	*/
		...
		if(cliente.getCarro().getTipo() == 1) {
			taxa +=taxa*0.15;
		}else if(cliente.getCarro().getTipo() == 2) {
			taxa +=taxa*0.20;
		}else if(cliente.getCarro().getTipo() == 3) {
			taxa +=taxa*0.30;
		}
