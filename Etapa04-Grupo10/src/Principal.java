...
public class Principal { //inicio da classe Principal
	...
	public static void main(String[] args) { //inicio do main
		...
		final int IMPRIMIR_PRECOS = 8;//designa numero constantes para cada operacao
		
		int comando; //inteiro que dita a operacao desejada
		...
		while(comando != CANCELAR) { //enquando sistema nao eh parado
			if(comando == ESTACIONAR) {
				...
				ClienteGaragem novoCliente = new ClienteGaragem(scan.next(), scan.next(), scan.next());
				...
				e1.imprimirDadosCliente(CPF); //imprime dados
			...
			else if(comando == IMPRIMIR_PRECOS) {
				
				e1.imprimeTabelaPrecos();
